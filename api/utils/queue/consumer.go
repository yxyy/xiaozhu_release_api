...
type Processor interface {
	Run(*Queue, Topic) error
}

type BatchProcessor interface {
	Run(*Queue, []*Topic) (redoMsg []string, err error)
}
...
func (q *Queue) AddJob(msg []string) {
	...
	go func() {
		...
		if q.processor != nil {
			if err := q.processor.Run(q, *topics[0]); err != nil {
				q.Log.Errorf("队列处理有误:%s，准备重新入队...", err)
				// 类型断言判断是否实现了 Retry 方法
				if retryProcessor, ok := q.processor.(interface{ Retry(*Queue, Topic) }); ok {
					retryProcessor.Retry(q, *topics[0]) // 调用 processor 自己的 Retry 方法
				} else {
					q.Retry(*topics[0])
				}
			}
		...
		if q.batchProcessor != nil {
			_, err := q.batchProcessor.Run(q, topics) // 返回重试消息的要自己为何re_try
			if err != nil {
				q.Log.Errorf("队列处理有误:%s，准备重新入队...", err)
				// 类型断言判断是否实现了 Retry 方法
				if retryProcessor, ok := q.processor.(interface{ Retry(*Queue, []*Topic) }); ok {
					retryProcessor.Retry(q, topics) // 调用 processor 自己的 Retry 方法
				} else {
...
func (q *Queue) Retry(topic Topic) {
...
func (q *Queue) restore2(msg []string) []string {
	var newMsg []string
	l := len(delimiter)
	for _, v := range msg {
		index := strings.Index(v, delimiter)
		if index == -1 {
			newMsg = append(newMsg, v)
		} else {
			newMsg = append(newMsg, v[index+l:])
		}
	}
...
func (q *Queue) restore(msg []string) []*Topic {
	newMsg := make([]*Topic, 0, len(msg))
	var topic Topic
	for _, v := range msg {
