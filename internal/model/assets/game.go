...
func (g Game) Create() error {
	return utils.MysqlDb.Model(&g).Create(&g).Error
}

func (g Game) Update() error {
	return utils.MysqlDb.Model(&g).Where("id", g.Id).Updates(&g).Error
}

func (g Game) List(params common.Params) (games []*GameInfo, total int64, err error) {
	tx := utils.MysqlDb.Model(&g).
	...
	err = tx.Offset(params.Offset).Limit(params.Limit).Find(&games).Error
...
func (g Game) GetAll() (game []*Game, err error) {
	...
	err = utils.MysqlDb.Model(&g).Find(&game).Error
	return
}
